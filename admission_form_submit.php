...
// echo $name."\n".$email."\n".$mob.$location."\n".$country."\n".$stream."\n".$Desired_Program."\n".$desired_intake_season."\n".$desired_intake_year."\n".$loan_requirement."\n".$loan_amount;
          ...
          if(!empty($name) && !empty($email) && !empty($mob) && !empty($location) && !empty($country) && !empty($stream) && !empty($desired_intake_season) && !empty($Desired_Program) && !empty($desired_intake_year) && !empty($loan_requirement) && !empty($loan_amount) && !empty($EntranceExam) && !empty($EntranceExamScore) && !empty($VerbalExam) && !empty($VerbalExamScore) && !empty($univname) && !empty($desired_intake_date))
          {
                $sql="SELECT * FROM `university_admission_apply` WHERE `STUDENT_EMAIL`='$email'";
                 ...
                 {
		                 $sql="INSERT INTO `university_admission_apply` (`STUDENT_NAME`, `STUDENT_EMAIL`, `STUDENT_MOBILE`, `CURRENT_LOCATION`, `EXPECTED_COUNTRY`, `EXPECTED_STREAM`, `DESIRED_PROGRAM`,`UNIVERSITY_REQUIRED`, `INTAKE_DATE` ,`DESIRED_INTAKE_SEASON`, `DESIRED_INTAKE_YEAR`,`ENTRANCE_EXAM`, `ENTRANCE_ExamScore`,`VERBAL_EXAM`, `VERBAL_ExamScore`, `LOAN_REQURED`, `AMOUNT`) VALUES ('$name', '$email', '$mob', '$location', '$country', '$stream', '$Desired_Program','$univname' ,'$desired_intake_date','$desired_intake_season', '$desired_intake_year', '$EntranceExam', '$EntranceExamScore', '$VerbalExam', '$VerbalExamScore', '$loan_requirement', '$loan_amount');";
